...
def Problem2(thickness,T2,time):        # 计算第2层厚度为thickness，最高温度
    ...
    plt.ylabel('Temperature/(Degree)',fontsize=10)
    plt.show()
...
def plot(alpha):
    ...
    plt.plot(X.numpy(),Predict.numpy(),'+',label='Predicted',mew=0.4)
    plt.plot(X.numpy(),Y.numpy(),"x",label='Ground Turth',mew=0.4)
    plt.xlabel('Time/(s)',fontsize=10)
    plt.ylabel('Temperature/(Degree)',fontsize=10)
...
def Distribution():
    ...
    def transfer(x):
        ...
        ans1 = mask1*(x/c1+thick0)
        ans2 = mask2*((x-5)/c2+thick1)
        ans3 = mask3*((x-8.6)/c3+thick2)
        ans4 = mask4*((x-14.6)/c4+thick3)
        return ans1+ans2+ans3+ans4
    ans = torch.zeros((len(ts)+1,500+1))
    ans[0,1:] = xs
    ans[1:,0] = ts
    Xs = transfer(xs).cuda()
    for i in range(500):
